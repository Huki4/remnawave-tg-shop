...
def _back_to_users(lang: str = None, i18n_instance=None):
    """Back button pointing to user management section."""
...
def _back_to_ban_mgmt(lang: str = None, i18n_instance=None):
    """Back button pointing to ban management section."""
    from aiogram.types import InlineKeyboardMarkup
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(
        text="Назад",
...
def get_user_card_keyboard(user_id: int, i18n_instance, lang: str,
    ...
    builder.row(InlineKeyboardButton(
        ...
        icon_custom_emoji_id="6021547434641987535"  # 🔍
    ), InlineKeyboardButton(
        text="Назад",
        callback_data="admin_section:user_management",
...
async def ban_user_prompt_handler(callback: types.CallbackQuery,
    ...
    try:
        await callback.message.edit_text(
            prompt_text,
            reply_markup=_back_to_ban_mgmt()
        )
    except Exception as e:
