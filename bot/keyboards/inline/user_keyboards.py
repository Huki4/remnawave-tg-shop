"""
user_keyboards.py — Полная структура меню бота.

НАСТРОЙКА КНОПОК:
  Весь текст кнопок — в словаре BUTTONS ниже.
  icon = custom_emoji_id (получить через @JsonDumpBot или @RawDataBot)
         Установите "" чтобы убрать иконку.
  Для премиум-эмодзи в тексте сообщений используйте формат:
...
# ════════════════════════════════════════════════════════
#  КОНФИГ КНОПОК
#  Меняйте text и icon здесь — больше нигде не нужно.
#  icon = custom_emoji_id (строка) или "" чтобы убрать иконку
# ════════════════════════════════════════════════════════
BUTTONS: dict = {
    # ── Главное меню ─────────────────────────────────────
    "buy":              {"text": "Оплата",                      "icon": "6030443364178992166"},
    "vpn":              {"text": "Управление подпиской",        "icon": "6023896773162967617"},
    "info":             {"text": "Информация",                  "icon": "5807800879553715710"},
    "referral":         {"text": "Рефералы",                    "icon": "6021690418398239007"},
    "gifts":            {"text": "Подарки",                     "icon": "6023826881160157558"},
    "settings":         {"text": "Настройки",                   "icon": "6021582331251268218"},
    "back":             {"text": "◀️ Назад",                    "icon": ""},
    "trial":            {"text": "Пробный период",              "icon": "6030462253445160459"},

    # ── Тарифы (Оплата) ──────────────────────────────────
    "plan_standard":    {"text": "Стандартный тариф",          "icon": "6024039683904772353"},
    "plan_family":      {"text": "Семейный тариф",             "icon": "6021690418398239007"},
    "plan_corp":        {"text": "Корпоративный тариф",        "icon": "6021650913289050282"},
    "plan_gift":        {"text": "Подписка в подарок 🎁",      "icon": "6023826881160157558"},

    # ── Информация ───────────────────────────────────────
    "info_channel":     {"text": "📢 Канал",                    "icon": ""},
    "info_terms":       {"text": "Пользовательское соглашение", "icon": "6021413766669801212"},
    "info_support":     {"text": "Техническая поддержка",       "icon": "5807800879553715710"},
    "info_privacy":     {"text": "Политика конфиденциальности", "icon": "6021413766669801212"},
    "info_faq":         {"text": "FAQ",                         "icon": "5807800879553715710"},

    # ── Рефералы ─────────────────────────────────────────
    "ref_share":        {"text": "📤 Поделиться ссылкой",       "icon": "6021690418398239007"},
    "ref_qr":           {"text": "📷 QR-код",                   "icon": "6021690418398239007"},

    # ── Подарки ──────────────────────────────────────────
    "gift_active":      {"text": "🟢 Активные подарки",         "icon": ""},
    "gift_inactive":    {"text": "⚪ Использованные подарки",   "icon": ""},
    "gift_buy":         {"text": "🎁 Купить подарок",           "icon": "6023826881160157558"},

    # ── Управление подпиской ─────────────────────────────
    "connect":          {"text": "🔗 Подключиться",             "icon": "6019290828759898301"},
    "my_devices":       {"text": "📱 Управление устройствами",  "icon": "6026029580907715757"},
    "sub_qr":           {"text": "📷 QR-код подписки",          "icon": ""},
    "vpn_extend":       {"text": "🔄 Продлить подписку",        "icon": "5807485774983077261"},

    # ── Настройки ────────────────────────────────────────
    "language":         {"text": "🌐 Сменить язык",             "icon": "6021582331251268218"},

    # ── Оплата ───────────────────────────────────────────
    "pay_yookassa":     {"text": "ЮKassa",                      "icon": "6030443364178992166"},
    "pay_crypto":       {"text": "CryptoBot",                   "icon": "6019290828759898301"},
    "pay_stars":        {"text": "Звёзды Telegram",             "icon": "6023826881160157558"},
    "pay_freekassa":    {"text": "FreeKassa",                   "icon": "6030443364178992166"},
    "pay_platega":      {"text": "Platega (СБП/карты)",         "icon": "6030443364178992166"},
    "pay_severpay":     {"text": "SeverPay",                    "icon": "6030443364178992166"},

    # ── Прочее ───────────────────────────────────────────
    "cancel":           {"text": "❌ Отмена",                   "icon": ""},
    "promo_inline":     {"text": "Промокод",                    "icon": "5807681883189812861"},
}


# ════════════════════════════════════════════════════════
#  ССЫЛКИ для раздела "Информация"
#  Вставьте сюда свои ссылки
# ════════════════════════════════════════════════════════
INFO_LINKS: dict = {
    "channel":  "https://t.me/yourchannel",        # Канал
    "terms":    "https://example.com/terms",        # Пользовательское соглашение
    "support":  "https://t.me/yoursupport",         # Техническая поддержка
    "privacy":  "https://example.com/privacy",      # Политика конфиденциальности
    "faq":      "https://example.com/faq",          # FAQ
}
...
def _btn(key: str, cb: str = None, url: str = None,
    ...
    cfg = BUTTONS.get(key, {"text": key, "icon": ""})
...
def get_subscription_options_keyboard(
    ...
    for months, price in sorted(items, key=lambda x: x[0] if x[0] else 0):
        ...
        if price is not None:
            if promo_discount_pct:
                label += f" — {price} {currency} (-{promo_discount_pct}%)"
            else:
                label += f" — {price} {currency}"

        if is_gift:
            cb = f"gift_period:{plan}:{months}"
        else:
            cb = f"sub:{plan}:{months}" if plan else f"sub:standard:{months}"

        builder.row(InlineKeyboardButton(text=label, callback_data=cb))

    if is_gift:
        builder.row(_back(cb="plan:gift"))
    else:
        builder.row(_back(cb="main_action:subscribe"))
...
def get_gift_detail_keyboard(lang: str, i18n_instance,
    ...
    back_cb = "gifts_action:active" if not is_used else "gifts_action:inactive"
    builder.row(_back(cb=back_cb))
    return builder.as_markup()
...
def get_my_subscription_keyboard(
    ...
    if has_active_sub and connect_url:
        extra = {}
        if BUTTONS["connect"]["icon"]:
            extra["icon_custom_emoji_id"] = BUTTONS["connect"]["icon"]
        builder.row(InlineKeyboardButton(
            text=BUTTONS["connect"]["text"],
            url=connect_url,
            **extra,
        ))
    ...
    if show_devices_button and has_active_sub:
        ...
        dev_text = f"{BUTTONS['my_devices']['text']} ({current_devices}/{dev_label})"
        extra = {}
        if BUTTONS["my_devices"]["icon"]:
            extra["icon_custom_emoji_id"] = BUTTONS["my_devices"]["icon"]
        builder.row(InlineKeyboardButton(
            text=dev_text,
...
def get_autorenew_confirm_keyboard(enable: bool, sub_id: int,
    ...
    builder = InlineKeyboardBuilder()
    if enable:
        builder.row(InlineKeyboardButton(text="✅ Да, включить",
                                         callback_data=f"autorenew:confirm:{sub_id}:1"))
    else:
        builder.row(InlineKeyboardButton(text="✅ Да, отключить",
                                         callback_data=f"autorenew:confirm:{sub_id}:0"))
...
def get_subscribe_only_markup(lang: str, i18n_instance) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.row(InlineKeyboardButton(text="Оформить подписку",
...
def get_payment_method_keyboard(lang: str, i18n_instance, months: int,
    ...
    builder = InlineKeyboardBuilder()
    method_map = {
        "yookassa":  ("ЮKassa",   f"pay:yookassa:{months}"),
        "cryptopay": ("CryptoBot", f"pay:cryptopay:{months}"),
        "stars":     ("Звезды",   f"pay:stars:{months}"),
        "freekassa": ("FreeKassa", f"pay:freekassa:{months}"),
        "platega":   ("Platega",   f"pay:platega:{months}"),
        "severpay":  ("SeverPay",  f"pay:severpay:{months}"),
    }
    for method in enabled_methods:
        if method in method_map:
...
def get_payment_methods_manage_keyboard(lang: str, i18n_instance,
    ...
    builder = InlineKeyboardBuilder()
    if has_card:
        builder.row(InlineKeyboardButton(text="💳 Мои карты",
